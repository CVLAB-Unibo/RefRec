...
class Classifier(pl.LightningModule):
    def __init__(self, dm, device):
        ...
        # if "shapenet" in hcfg("dataset_source.name"):
        # print("weights:", dm.train_ds.class_weights)
        # self.loss_fn = get_loss_fn(dm.train_ds.class_weights)
        # else:
    ...
    def on_train_start(self):
        ...
        if hcfg("use_proto"):
            ...
            self.protoypes = np.load(f"prototypes_supp/{project_name}_proto_perc.npy")
    ...
    def training_step(self, batch, batch_idx):
        ...
        # if hcfg("use_proto"):
            # source_embeddings = F.normalize(embedding.squeeze(2), dim=1)
        #     centroid_loss = 0            
        #     predictions = torch.argmax(logits, dim=1)
        #     for c in range(self.hparams.num_classes):
        #         if torch.sum(labels == c)>0:
        #             centroid_loss += torch.linalg.norm(source_embeddings[labels == c]- self.protoypes[c], dim=1).mean()
        #     centroid_loss = centroid_loss*0.01
        #     loss += centroid_loss
        ...
        diff_proto = embeddings_s.unsqueeze(1).repeat(1,self.hparams.num_classes,1).detach() - self.protoypes
        ...
        # for c in range(self.hparams.num_classes):
        #     if torch.sum(predictions_teacher == c)>0:                
        #         self.protoypes[c] = (self.protoypes[c] * 0.999) +  (embeddings_t[predictions_teacher == c].mean(dim=0) * (1-0.999))
